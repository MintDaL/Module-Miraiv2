module.exports = function({ api, models }) {
  ...
  const { ADMINBOT, PREFIX, BOTNAME } = global.config
  const { threadInfo, threadData } = global.data;
    ...
    try {
      ...
      if (!oa[event.threadID]) {
        ...
        if (oa[event.threadID] === true) {
          ...
          console.log({adminIDs,senderID: event.senderID})
